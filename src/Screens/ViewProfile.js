...
import { View, StyleSheet, Text, Modal, TouchableOpacity, ImageBackground, Image, ScrollView } from "react-native"; 
...
const fetchDetails= () => {
 userDocRef = doc(db,"Users", mateId);
  ...
  <Text style={{fontSize:20,fontWeight:"bold"}} > {calculateAge(allData.dateOfBirth)} years old {allData.gender} </Text>
...
/>
<View style={styles.Card}>
  <Text style={{fontSize:20,fontWeight:"bold"}} >Authors that got me hooked</Text>
  <Text>
  {allData.favAuthors.join(",  ")}
    </Text>
      </View>
...
<View style={styles.Card}>
  <Text style={{fontSize:20,fontWeight:"bold"}} >Genres that keep me going</Text>
  <Text>
  {allData.favGenres.join(",  ")}
    </Text>
      </View>
      <Image 
