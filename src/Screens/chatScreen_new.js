import React, {
  ...
  const ChatDisplay_new = ({ route, navigation }) => {
    ...
    // Fetch or create chat between users via the backend
    const getChatId = async () => {
      console.log("Creating chat via backend...");
      try {
        ...
        return data.chatId; // Return the chatId from the backend
      } catch (error) {
        console.error("Error creating chat:", error);
        Alert.alert("Error", "Could not create or find the chat.");
        return null;
      }
    };
    ...
    const onSend = useCallback(
      async (newMessages = []) => {
        ...
        // If no chatId, create the chat now
        if (!currentChatId) {
          // getChatId will create a new chat if needed
        ...
        try {
          ...
          if (message.image && message.pending) {
            ...
            const tempMessageId = `${Date.now()}_${Math.random()
              .toString(36)
              .substr(2, 9)}`;
            const imagePath = `images/${currentChatId}/${tempMessageId}.jpg`;
    ...
    // Add this useEffect to check if a chat exists between the current user and the other participant
    useEffect(() => {
      const checkChatExists = async () => {
        try {
          const idToken = await auth.currentUser.getIdToken();
          const response = await fetch(`${SERVER_URL}/chat-list/${senderId}/chat`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${idToken}`,
          ...
          if (data && data !== false) {
            ...
            setChatId(null);
            setAllData(null); // or setAllData({}) if you prefer
          }
        } catch (error) {
      ...
      checkChatExists();
      // Only run on mount or when userId/allData.id changes
    }, [userId]);
    ...
    return (
      <View style={styles.container}>
        ...
        {/* Show notice if no chat exists yet */}
        {allData === false && (
          <View style={{ padding: 16, backgroundColor: theme.colors.background }}>
            <Text style={{ color: theme.colors.text, fontSize: moderateScale(16), textAlign: 'center' }}>
              No chat exists yet. Your first message will start a new chat.
            </Text>
          </View>
        )}
        <GestureHandlerRootView>
  ...
  export default ChatDisplay_new;
